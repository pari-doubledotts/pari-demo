...
const {merchantApplicationEmail,merchantWelcome,merchantSignup } = require('../../config/template');
...
router.post('/add', async (req, res) => {
  try {
    ...
    let textMsgData = merchantApplicationEmail()
    await sendMail(email, textMsgData);
    // await sendMail.sendEmail(email, 'merchant-application');
...
const createMerchantUser = async (email, name, merchant, host) => {
  ...
  if (existingUser) {
    ...
    let textMsgData = merchantWelcome({name:firstName})
    await sendMail(email, textMsgData);
    // await sendMail.sendEmail(email, 'merchant-welcome', null, name);
    ...
    let textMsgData = merchantSignup(host, { resetToken:resetToken, email})
    await sendMail(email, textMsgData);
    // await sendMail.sendEmail(email, 'merchant-signup', host, {
    //   resetToken,
    //   email
    // });
