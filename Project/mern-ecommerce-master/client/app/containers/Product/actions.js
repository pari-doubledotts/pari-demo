...
export const productChange = (name, value) => {
  let formData = {};
  formData[name] = value;
  //console.log("productChange==========>>", formData);
  return {
    type: PRODUCT_CHANGE,
    payload: formData
  };
...
export const productEditChange = (name, value) => {
  let formData = {};
  formData[name] = value;
  console.log("productEditChange==========>>", formData);
  return {
...
export const resetProduct = () => {
  return async (dispatch, getState) => {
    await dispatch({ type: RESET_PRODUCT });
  };
};
...
export const addProduct = () => {
  return async (dispatch, getState) => {
    try {
      ...
      console.log("PRODUCT FROM DATA=======>>", getState().product.productFormData);
      const user = getState().account.user;
      const brands = getState().brand.brandsSelect;
      const brand = unformatSelectOptions([product.brand]);

      //console.log("IMAGES=========1212===>>>", product.image);
      //console.log("COLOR=========1212===>>>", product.color);
      const newProduct = {
        sku: product.sku,
        name: product.name,
      ...
      const { isValid, errors } = allFieldsValidation(newProduct, rules, {
        ...
        'required.size': 'Please, add your Product size',
        'required.color': 'Please, choose color',
  ...
  // FORMDATA
      ...
      //console.log("New-Produact", newProduct);
            ...
            if (key === 'brand' && newProduct[key] === null) {
              ...
              if (key == 'image') {
                for (let i = 0; i < newProduct.image.length; i++) {
                  const element = newProduct.image[i];
                  formData.append(key, element)
                }
              } else if (key == 'size') {
                for (let i = 0; i < newProduct.size.length; i++) {
                  const element = newProduct.size[i];
                  formData.append(key, element)
                }
              } else {
      ...
      //console.log("xxxxx", successfulOptions);
...
export const updateProduct = () => {
  return async (dispatch, getState) => {
    try {
      ...
      console.log("PRODUCT UPDATE=======>>", getState().product.product);
      const brand = unformatSelectOptions([product.brand]);
      ...
      console.log("UPDATE_Image", product.image);
      console.log("UPDATE_PRODUCT", newProduct);
      ...
      console.log("PUTSUCCESS MASSAGE", successfulOptions);
